...
@interface MRadioButton()
-(void)defaultInit;
-(void)otherButtonSelected:(id)sender;
-(void)handleButtonTap:(id)sender;
@end

@implementation MRadioButton
...
+(void)registerInstance:(MRadioButton*)radioButton withGroupID:(NSString *)aGroupID{
...
+(void)buttonSelected:(MRadioButton*)radioButton{
    ...
    if (rb_instances) {
        for (int i = 0; i < [rb_instances count]; i++) {
            MRadioButton *button = [rb_instances objectAtIndex:i];
...
-(void)handleButtonTap:(id)sender{
    [_button setSelected:YES];
    [MRadioButton buttonSelected:self];
}
...
-(void)defaultInit{
    ...
    [_button setImage:[UIImage imageNamed:@"btn_uncheck"] forState:UIControlStateNormal];
    [_button setImage:[UIImage imageNamed:@"btn_check_light"] forState:UIControlStateSelected];
 ...
 //   [RadioButton registerInstance:self];
    ...
    // update follow:
    [MRadioButton registerInstance:self withGroupID:self.groupId];
