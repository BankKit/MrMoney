...
@implementation MWithdrawAction
...
-(void)onRequestFinishResponse:(ASIHTTPRequest*)request{
    ...
    if ([MActionUtility isRequestJSONSuccess:l_dict_response]) {
        ...
        if (result == 0) {
            ...
            if ([(UIViewController*)m_delegate  respondsToSelector:@selector(onResponseWithdrawSuccess)]) {
                [m_delegate onResponseWithdrawSuccess];
            }
            
        }else {
            if ([(UIViewController*)m_delegate  respondsToSelector:@selector(onResponseWithdrawFail)]) {
                [m_delegate onResponseWithdrawFail];
            }
        }
